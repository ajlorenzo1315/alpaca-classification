...
def get_file_2(path_file):  
  ...
  with open(path_file) as f:
      data = json.load(f)
  #print(data.keys())
  dict_text={key: data[key] for key in data.keys() if key in {'title','1024_text'} }
  #try:
...
def get_all_files(path_data,max_cont=-1):
    ...
    if len(classes)>0:
     ...
     # Paso adicional para ordenar las clases por cantidad de archivos
    class_counts = [(class_name, len(archiver_per_clases[class_name])) for class_name in archiver_per_clases]
    class_counts.sort(key=lambda x: x[1], reverse=True)  # Ordena de mayor a menor cantidad de archivos
    sorted_classes = [class_name for class_name, _ in class_counts]

    return archiver_per_clases,prompt_per_clases,promt_all_reson,archiver_all_reson,sorted_classes
...
log_filename = "resultados_llm2_test_promt_4_text_etiquetas_1024.txt"
logging.basicConfig(filename=log_filename, level=logging.INFO, format="%(asctime)s - %(levelname)s - %(message)s")

#
train,promt_train,b,d,classes=get_all_files('../data/splits/train_new_short',-1)
...
#    guardar_en_csv('resultados_llm2_3.csv', datos_a_guardar_1) 
    guardar_en_json(f'resultados_llm2_test_promt_4_text_etiquetas_1024_{model_name}.json', datos_a_guardar_1)
