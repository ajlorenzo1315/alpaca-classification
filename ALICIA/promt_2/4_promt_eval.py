...
def creating_promt_5_0(class_name,text_classification):
  class_name=(', ').join(class_name)
  #text_classification='\n'.join([text_classification[key] for key in text_classification])
  return f"Classify the following text: '{text_classification}' in {class_name} Reply with only one word"

def creating_promt_5(class_name,text_classification):
  class_name=(', ').join(class_name)
  text_classification='\n'.join([str(text_classification[key]) for key in text_classification])
  return f"Classify the following text: '{text_classification}' in {class_name} Reply with only one word"
...
log_filename = "llama_eval_log_mistral.txt"
...
#logging.info(query2)
#
model_name='mistral-7b-v0.1.Q8_0'
model_path = f"../llama_models/{model_name}.gguf"
...
prompts=[(name,creating_promt_3(classes,b_2,text_class[0],keys_wod),creating_promt_2(classes,text_class[0])) for name,text_class in zip(c,a)]
#prompts=[(name,creating_promt_5(classes,text_class[0]),creating_promt_2(classes,text_class[0])) for name,text_class in zip(c,a)]
...
#    guardar_en_csv('resultados_llm1_2.csv', datos_a_guardar_1)   
    guardar_en_csv(f'resultados_llm2_promt_2_model_{model_name}.csv', datos_a_guardar_1) 
    guardar_en_json(f'resultados_llm2_promt_2_model_{model_name}.json', datos_a_guardar_1)
    guardar_en_json(f'resultados_llm2_not_promt_2_model_{model_name}.json', datos_no_promting)   
