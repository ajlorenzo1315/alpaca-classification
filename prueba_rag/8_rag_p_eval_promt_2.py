...
def get_file_2(path_file):  
  ...
  try:
    dict_text['keysword']=data['keyword_frequency_kebert']
  except:
    print(data.keys())
...
def creating_promt_3(class_name,text_classification,keywords):
  class_name=(', ').join(class_name)
...
#print(promt)
# Configura el archivo de registro
log_filename = "llama_rag_eval_log_2.txt"
logging.basicConfig(filename=log_filename, level=logging.INFO, format="%(asctime)s - %(levelname)s - %(message)s")
...
test,promt_test,a,c=get_all_files('../data/splits/train_new_data',10)
#test,promt_test,a,c=get_all_files('./data/splits/train_new',2)
prompts=[(name,creating_promt(classes,b_2,text_class[0]),creating_promt_3(classes,text_class[0],keys_wod)) for name,text_class in zip(c,a)]
_,_,text_field=prompts[0]
...
for prompt in tqdm.tqdm(prompts,desc="promting"):
    ...
    guardar_en_json('resultados_rag1_test_promt_2_keysword.json', datos_a_guardar_1)   
