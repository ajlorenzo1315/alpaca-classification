...
for i,example in enumerate(train[class_selected]):
  ...
  creating_promt_2(classes,text_class[0])) for name,text_class in zip(c,a) if text_class[1]==class_selected and name in archives_pobe ]
  
  #print(prompts[0])
  datos_a_guardar_1 = []

  datos_no_promting=[]
  name_examaple_text=os.path.splitext(os.path.basename(example))[0]
  ...
  for prompt in tqdm.tqdm(prompts,desc="promting"):
      ...
      guardar_en_json(f'resultados_llm2_{class_selected}_{model_name}.json', datos_a_guardar_1)
